...
export function AuthProvider({ children, onLogout }: { children: ReactNode; onLogout?: () => void }) {
  ...
  const loginMutation = useMutation({
    ...
    onError: (error: Error) => {
      // Show error message from server (with attempts remaining if present)
      toast({
        title: "Login failed",
  ...
  const logoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (onLogout) onLogout(); // Call onLogout callback if provided
    },
    onError: (error: Error) => {
      toast({
        title: "Logout failed",
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        logoutMutation,
        registerMutation,
        onLogout, // Provide onLogout in context
      }}
    >
      {children}
    </AuthContext.Provider>
